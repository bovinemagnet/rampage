...
- [ ] After parsing each YAML, a post-processing step walks the model and expands placeholders in string fields:
...
- [ ] Unresolved placeholders cause a validation failure (consistent with F-002).
- [ ] Expansion is applied to: env headers, run-level headers, scenario headers, scenario `description`, request `bodyTemplate`, and any other string field where it makes sense (whitelisted to avoid unintended substitution in GraphQL queries).
- [ ] Escaped placeholders (`\${...}`) pass through literally.
- [ ] Tests cover each placeholder type and the escape syntax.
