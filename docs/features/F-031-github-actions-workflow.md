...
- [ ] `.github/workflows/ci.yml` runs on push and pull request to `main`:
  ...
  - Run `./gradlew build test` (using the wrapper, not `gradle21w`, since CI does not have the user's symlink).
...
- Use the wrapper (`./gradlew`), not `gradle21w`. The user's CLAUDE.md preference is for local development.
