...
- [ ] `strategy: queue` — each row is consumed at most once. When the queue drains, the scenario stops emitting requests (or fails, depending on a new `onExhaustion: stop|fail` field; default `stop`).
- [ ] `strategy: shuffle` — the dataset is shuffled once at preload, then iterated once (queue-like) or circularly (current behaviour), depending on `onExhaustion`.
- [ ] `strategy: random` — each pull selects a random row from the dataset (not "shuffle once").
- [ ] `strategy: circular` — unchanged.
- [ ] Tests cover each strategy with `onExhaustion` set to both values.
- [ ] Documented in feature-table form in the Antora docs (F-033 will migrate it).
