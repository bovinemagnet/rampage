...
NOTE: This is a reference snapshot of `docs/roadmap/roadmap.md`. It is not updated automatically. Last updated: 2026-05-15 — REST/HTTP support, multi-step scenarios, response extraction, inter-step pauses, and rich checks (regex, header, responseTime, bodyContains) shipped. CI quality gate (`summariseRun`, `load-test-pr.yml`), HAR importer (`importHar`), and OpenAPI importer (`importOpenApi`) also shipped.
...
PRD Phase 2 (YAML Loading) is complete; Phases 3–5 are partially implemented; Phase 6 (Reporting and CI) has only the disconnected `RunMetadataWriter`. See xref:reference/requirements-traceability.adoc[the requirements traceability] for the line-by-line status.
...
The following capabilities shipped after M4: REST/HTTP-method scenarios, multi-step scenarios with per-step `endpointRef`, response extraction (`extract:`), inter-step pauses (`pauseAfter:`), and richer check types (regex, header, `responseTimeMillis`, `bodyContains`, `equalsValue`, `saveAs`). These are not yet reflected in the milestone feature list below; they will be formalised in a future milestone update.
